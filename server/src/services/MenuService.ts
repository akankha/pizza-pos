...
import type { MenuData, PizzaSize, Crust, Topping, MenuItem } from '../../../shared/types/index.js';
...
export class MenuService {
  async getMenuData(): Promise<MenuData> {
    ...
    const [toppings] = await db.query('SELECT * FROM toppings ORDER BY category, name');
    const [sides] = await db.query('SELECT * FROM menu_items WHERE category = ? ORDER BY price', ['side']);
    const [drinks] = await db.query('SELECT * FROM menu_items WHERE category = ? ORDER BY price', ['drink']);
    const [specialtyPizzas] = await db.query('SELECT * FROM specialty_pizzas WHERE active = 1 ORDER BY name');
    const [combos] = await db.query('SELECT * FROM combo_deals WHERE active = 1 ORDER BY category, price');
