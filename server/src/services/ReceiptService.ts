...
export class ReceiptService {
  ...
  static async generatePDFReceipt(
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=receipt-${data.orderId}.pdf`
    );
    doc.pipe(res);
    ...
    if (data.pst !== undefined && data.pst > 0) {
      doc.text(
        `${settings.tax_label_pst} (${(settings.pst_rate * 100).toFixed(1)}%):`,
        { continued: true }
      );
  ...
  static async calculateTaxes(
    subtotal: number
  ): Promise<{ gst: number; pst: number; total: number }> {
    const settings = await this.getSettings();
