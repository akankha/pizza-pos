...
import { authenticateToken, requireRestaurantAdmin, requireSuperAdmin } from '../middleware/auth.js';
...
router.put('/', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
    ...
    if (!restaurant_name || !restaurant_address || !restaurant_city || !restaurant_phone) {
...
router.get('/printer/status', authenticateToken, requireRestaurantAdmin, async (req, res) => {
  try {
    const status = await PrinterService.checkStatus();
    res.json({ success: true, data: status });
  } catch (error: any) {
    console.error('Printer status error:', error);
    res.status(500).json({ success: false, error: 'Failed to check printer status' });
  }
});

// Test print (Restaurant Admin and above)
router.post('/printer/test', authenticateToken, requireRestaurantAdmin, async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to print test receipt' });
  }
});

// Get available printers (Restaurant Admin and above)
router.get('/printer/list', authenticateToken, requireRestaurantAdmin, async (req, res) => {
  try {
