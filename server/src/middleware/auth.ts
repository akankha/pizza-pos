...
export const requireRestaurantAdmin = (req: Request, res: Response, next: NextFunction) => {
...
export const requireReception = (req: Request, res: Response, next: NextFunction) => {
...
export const requireKitchen = (req: Request, res: Response, next: NextFunction) => {
  const allowedRoles = ['super_admin', 'restaurant_admin', 'reception', 'kitchen'];
  if (!req.user?.role || !allowedRoles.includes(req.user.role)) {
    return res.status(403).json({ 
      success: false, 
