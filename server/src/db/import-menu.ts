...
/**
 * Custom Menu Import Script
 * 
 * Replace the data below with your actual menu items from the PDF
 */

interface MenuData {
  sizes: Array<{ id: string; name: string; display_name: string; base_price: number }>;
  crusts: Array<{ id: string; type: string; display_name: string; price_modifier: number }>;
  toppings: Array<{ id: string; name: string; price: number; category: 'meat' | 'veggie' | 'cheese' }>;
  sides: Array<{ id: string; name: string; price: number; description: string }>;
  drinks: Array<{ id: string; name: string; price: number; description: string }>;
  ...
  try {
    ...
    for (const size of menuData.sizes) {
      await connection.query(
        ...
        [size.id, size.name, size.display_name, size.base_price]
      );
    ...
    for (const crust of menuData.crusts) {
      await connection.query(
        ...
        [crust.id, crust.type, crust.display_name, crust.price_modifier]
      );
    ...
    for (const topping of meatToppings) {
      await connection.query(
        ...
        [topping.id, topping.name, topping.price, topping.category]
      );
    ...
    for (const side of menuData.sides) {
      await connection.query(
        ...
        [side.id, side.name, 'side', side.price, side.description]
      );
    ...
    for (const drink of menuData.drinks) {
      await connection.query(
        'INSERT INTO menu_items (id, name, category, price, description) VALUES (?, ?, ?, ?, ?)',
        [drink.id, drink.name, 'drink', drink.price, drink.description]
      );
    ...
    for (const pizza of menuData.specialtyPizzas) {
      await connection.query(
        'INSERT INTO specialty_pizzas (id, name, description, toppings, price_small, price_medium, price_large, price_xlarge) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        [pizza.id, pizza.name, pizza.description, pizza.toppings, pizza.price_small, pizza.price_medium, pizza.price_large, pizza.price_xlarge]
      );
    ...
    for (const combo of menuData.combos) {
      await connection.query(
        ...
        [combo.id, combo.name, combo.description, combo.price, combo.items, combo.category]
      );
