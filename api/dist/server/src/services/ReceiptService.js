...
class ReceiptService {
     ...
     */
    static async generatePDFReceipt(data, res) {
        const settings = await this.getSettings();
        const doc = new pdfkit_1.default({ size: [226.77, 841.89], margin: 20 }); // 80mm thermal printer width
        // Pipe to response
        ...
        doc
            .fontSize(16)
            .font("Helvetica-Bold")
            .text(settings.restaurant_name, { align: "center" });
        doc
            .fontSize(9)
            .font("Helvetica")
            .text(settings.restaurant_address, { align: "center" });
        ...
        doc.moveDown(0.5);
        try {
            const qrData = await qrcode_1.default.toDataURL(`ORDER:${data.orderId}`);
            const qrImage = Buffer.from(qrData.split(",")[1], "base64");
            doc.image(qrImage, doc.page.width / 2 - 40, doc.y, {
                width: 80,
                height: 80,
            });
            doc.moveDown(6);
     ...
     */
    static async generateTextReceipt(data) {
        const settings = await this.getSettings();
        let receipt = "";
        // Header
        receipt += "=".repeat(48) + "\n";
     ...
     */
    static async calculateTaxes(subtotal) {
        const settings = await this.getSettings();
     ...
     */
    static async calculateTotals(items, discountPercent) {
