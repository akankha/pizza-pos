...
async function migrateOrderSchema() {
    ...
    `, [process.env.DB_NAME || 'pizza_pos']);
        const ordersColumnNames = ordersColumns.map(col => col.COLUMN_NAME);
        // Add subtotal column
    ...
    `);
        const ordersIndexNames = ordersIndexes.map(idx => idx.Key_name);
    ...
    `, [process.env.DB_NAME || 'pizza_pos']);
        const itemsColumnNames = itemsColumns.map(col => col.COLUMN_NAME);
    ...
    `);
        const itemsIndexNames = itemsIndexes.map(idx => idx.Key_name);
    ...
    `);
        if (existingItems.length > 0) {
            console.log(`  ℹ️  Migrating ${existingItems.length} existing custom pizza items...`);
