  ...
  if (loading) {
    return (
      <div className="h-screen w-screen flex items-center justify-center bg-gray-100">
        <div className="text-3xl font-bold text-[#FF6B35]">Loading orders...</div>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto">
          {orders.length === 0 ? (
            <div className="text-center py-20">
              ...
              <h2 className="text-4xl font-bold text-gray-400">No Active Orders</h2>
              ...
              {orders.map((order) => (
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className={`px-4 py-2 rounded-full text-sm font-bold flex items-center gap-2 shadow-md ${
                      ...
                      {order.status === 'pending' && <Clock size={16} aria-hidden="true" />}
                      {order.status === 'ready' && <CheckCircle size={16} aria-hidden="true" />}
                        ...
                        {item.customPizza && (
                          <div className="ml-4 text-xs text-gray-500 mt-1 space-y-0.5">
                            ...
                            {item.customPizza.toppings.length > 0 && (
                              <div>• Toppings: {item.customPizza.toppings.length}</div>
                            )}
                          </div>
                        )}
                      </div>
                  ...
                  <div className="border-t-2 border-gray-200 pt-4">
                    ...
                    {/* Receipt Button */}
                    <TouchButton
                      onClick={() => window.open(`/api/orders/${order.id}/receipt/pdf`, '_blank')}
                      variant="secondary"
                      size="medium"
                      fullWidth
