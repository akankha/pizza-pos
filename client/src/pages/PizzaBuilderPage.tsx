...
export default function PizzaBuilderPage() {
  ...
  const handleAddToCart = () => {
    ...
    const totalPrice = sizeData.basePrice + crustData.priceModifier + toppingPrices;

    addItem({
      type: 'custom_pizza',
      name: `Custom ${sizeData.displayName} Pizza`,
      price: totalPrice,
      quantity: 1,
      customPizza: {
        size: selectedSize,
    ...
    <div className="h-screen w-screen bg-gray-100 flex flex-col">
      ...
      <div className="bg-white border-b border-gray-200 p-6 shadow-sm">
        <div className="flex justify-between items-center max-w-7xl mx-auto">
          ...
          <h1 className="text-4xl font-bold text-gray-800">🍕 Build Your Pizza</h1>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 md:space-y-8">
              ...
              {menuData.sizes.map((size) => (
                ...
                >
                  <div className="flex justify-between items-center">
                    <span className="font-bold text-xl">{size.displayName}</span>
                    <span className={`font-bold text-lg ${selectedSize === size.name ? 'text-white' : 'text-[#FF6B35]'}`}>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {menuData.crusts.map((crust) => (
                ...
                >
                  <div className="flex justify-between items-center">
                    <span className="font-bold text-xl">{crust.displayName}</span>
                    <span className={`font-bold text-lg ${selectedCrust === crust.type ? 'text-white' : 'text-[#FF6B35]'}`}>
          ...
          <div>
            <h2 className="text-3xl font-bold text-gray-800 mb-4 flex items-center gap-2">
              ...
              <span className="text-xl font-normal text-gray-500">({selectedToppings.length} selected)</span>
            </h2>
                      ...
                      return (
                        ...
                        >
                          <div className="text-center">
                            <div className="font-semibold mb-1">{topping.name}</div>
                            <div className={`text-sm ${isSelected ? 'text-white' : 'text-[#10B981]'}`}>
        ...
        <div className="w-full lg:w-96 bg-white/80 backdrop-blur-sm border-t lg:border-t-0 lg:border-l border-gray-200 p-4 md:p-8 flex flex-col shadow-lg">
          ...
          <div className="flex-1 overflow-y-auto space-y-4 mb-4">
            ...
            <div className="bg-gray-50 rounded-2xl p-4">
              <div className="text-sm text-gray-600 font-semibold mb-1">Size</div>
              <div className="text-xl font-bold text-gray-800">
                {selectedSize ? menuData.sizes.find(s => s.name === selectedSize)?.displayName : 'Not selected'}
              </div>
            </div>

            {/* Crust */}
            <div className="bg-gray-50 rounded-2xl p-4">
              <div className="text-sm text-gray-600 font-semibold mb-1">Crust</div>
              <div className="text-xl font-bold text-gray-800">
                {selectedCrust ? menuData.crusts.find(c => c.type === selectedCrust)?.displayName : 'Not selected'}
              </div>
            </div>

            {/* Toppings - scrollable */}
            <div className="bg-gray-50 rounded-2xl p-4 max-h-64 overflow-y-auto">
              <div className="text-sm text-gray-600 font-semibold mb-2">Toppings ({selectedToppings.length})</div>
              <div className="flex flex-wrap gap-2">
                {selectedToppings.length === 0 ? (
                  ...
                  selectedToppings.map(toppingId => {
                    const topping = menuData.toppings.find(t => t.id === toppingId);
                    return (
                      <span key={toppingId} className="bg-gradient-to-r from-green-500 to-emerald-500 text-white px-3 py-1 rounded-full text-sm font-semibold shadow">
                        {topping?.name}
                      </span>
                    );
                  })
